...
def type_of_card(card) -> str:
    if result := transform_joker(card):
        # print(card, result)
        return result
    card_labels = {}
    for char in card:
...
def transform_joker(card: str) -> str:
    joker = "J"
    if card == "JJJJJ":
        return HAND_STRENGTH_ORDER[6]
    if joker in card:
        ...
        for char in card_list:
            if char == joker:
                continue
            if CARDS_STRENGTH.index(char) in card_dict:
                card_dict[CARDS_STRENGTH.index(char)] += 1
            else:
                card_dict[CARDS_STRENGTH.index(char)] = 1
        if max(card_dict.values()) == 4:
            return HAND_STRENGTH_ORDER[6]
        elif max(card_dict.values()) == 3 and len(card_dict) == 3:
            return HAND_STRENGTH_ORDER[6]
        elif max(card_dict.values()) == 2 and len(card_dict) == 2:
            if list(card_dict.values()).count(2) == 1:
                return HAND_STRENGTH_ORDER[5]
            return HAND_STRENGTH_ORDER[4]
        elif max(card_dict.values()) == 1 and len(card_dict) == 1:
            return HAND_STRENGTH_ORDER[6]
        elif max(card_dict.values()) == 3 and len(card_dict) == 4:

            return HAND_STRENGTH_ORDER[5]
        elif max(card_dict.values()) == 2 and len(card_dict) == 3:
            if list(card_dict.values()).count(2) == 1:
                return HAND_STRENGTH_ORDER[3]
            else:
                return HAND_STRENGTH_ORDER[4]
            ...
            return HAND_STRENGTH_ORDER[5]
        elif max(card_dict.values()) == 1 and len(card_dict) == 4:
            return HAND_STRENGTH_ORDER[1]
        elif max(card_dict.values()) == 1 and len(card_dict) == 3:
            return HAND_STRENGTH_ORDER[3]
        elif max(card_dict.values()) == 1 and len(card_dict) == 2:
            return HAND_STRENGTH_ORDER[5]
        elif max(card_dict.values()) == 3 and len(card_dict) == 2:
            return HAND_STRENGTH_ORDER[5]
        elif max(card_dict.values()) == 3 and len(card_dict) == 1:
            return HAND_STRENGTH_ORDER[6]
        elif max(card_dict.values()) == 2 and len(card_dict) == 1:
            return HAND_STRENGTH_ORDER[6]
